    ...
    //This path should point to Composer's autoloader from where your MongoDB library will be loaded
      ...
      // when using default settings
        ...
        if (isset($_GET['tbname'])&&isset($_GET['edid'])) {
          ...
          echo "<div class=\"Tbname_andbtn\"><h3>" . $cur_tb . " Edit <a href=\"table.php?tbname=" . $cur_tb . "\"><button type=\"button\" class=\"left_arrow\"> Back</button></a></h3></div>";
          echo "<div class=\"cardsbd\">
              <div class=\"card\">
                <div class=\"container\">";
          $tbQuery = array('_id' => new MongoDB\BSON\ObjectId($cur_id));
          $cursor = $collection->find($tbQuery);
          $array = iterator_to_array($cursor);
          $limit = 0;
          echo "<form action=\"dbEditDoc.php?tbname=". $cur_tb ."&edid=". $cur_id ."\" method=\"post\">";
          foreach ($array as $k => $v) {
            foreach ($v as $a => $b) {
              if ($limit<1) {
                if ($a!="_id") {
                  echo "<p>". $a .": <input type=\"text\" name=\"". $a ."\" value=\"" . $b . "\"/></p>";
                }
              }
            }
            $limit=1;
